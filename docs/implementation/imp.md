...
> You must strictly adhere to the 3-step operational protocol defined in [`CLAUDE.md`](file:///Users/prathambansal/dev/rapp/CLAUDE.md#L10-L28):
> 1. **Read First**: Check the active stage and task status below before modifying code.
...
> 3. **Log & Update Before Exiting**: Check off (`[x]`) completed items and append a session log entry in **Section 6** below before terminating.
...
#### 2.1 Results Screen Hero Redesign & Outcome Capture Survey (Antigravity, Gemini Pro)
...
#### 2.2 Social Proof Aggregation Query & Surfacing (Antigravity, Gemini Pro)
