...
**Progress: 15 / 15 items complete (100%)** — Server-Side Chat Rate Limiting, Real Email Delivery, Task Block 1 (MoR Swap + Annual Pass), Block 2 (Safety Redirect + ToS/Privacy), Block 3 (Results Hero Redesign + Social Proof Badge), Block 4 (Readiness Check, Bail-Out Frames, Skill Leveling), Block 5 (Photo Checkpoints Mid-Repair, Check My ChatGPT Answer Funnel), Block 7 (Maintenance Content, Knowledge Hub), Block 8 (Referral Program, Recall/TSB Watch Alerts). Update this line whenever a row below flips to ✅. Remaining backlog item (On-Site Ingestion, Stage 4.1) is prepped and ready to run — see `docs/onsite_ingestion_runbook.md`.
...
| **[x] Photo Checkpoints Mid-Repair** | Trust Moat & OCR | §4, §11 Stage 2 #8 | ✅ **COMPLETED (Block 5)** |
| **[x] Skill Leveling (Competence Tracking)** | Personalization & Retention | §4 Stage 2 #8.5 | ✅ **COMPLETED (Block 4)** |
| **[x] "Check My ChatGPT Answer" Funnel** | Acquisition Funnel | §4 Stage 2 #8.6 | ✅ **COMPLETED (Block 5)** |
...
| **[x] Referral Program** | Growth | §7, Stage 3 #11 | ✅ **COMPLETED (Block 8)** |
| **[x] Recall / TSB Watch alerts** | Value & Retention | §7, Stage 3 #12 | ✅ **COMPLETED (Block 8)** |
| **On-Site Ingestion (15 generations)** | Database Scale | §7, Stage 4 | 🔴 **GTM Gate** — prepped, ready to run on second laptop (`docs/onsite_ingestion_runbook.md`) |
...
#### 2.4 [x] Photo Checkpoints Mid-Repair (Claude Code, Sonnet)
...
#### 2.6 [x] "Check My ChatGPT Answer" Verification Funnel (Antigravity, Gemini Pro)
...
#### 3.3 [x] Referral Program & Recall Alerts (Jules)
- **Implementation**:
...
#### 4.1 [ ] 15-Generation On-Site Batch Ingestion (was: 20-Vehicle Jules Pilot)
- **Pivot (2026-07-15)**: this is now an **on-site run on a more capable laptop**, not Google Jules. Jules proved unreliable in practice, and a laptop the owner controls can plug the SSD in and write straight into the live store — no Git-LFS round-trip. Logged in `docs/UPDATED_PRODUCT_NORTH_STAR.md` §12.
- **Goal**: Ingest NHTSA TSB data for a curated **15 vehicle generations** (highest US on-road population + highest repair-cost / known-issue fraud-shield value — see North Star §1/§3) into the `chroma_db` vector store on the external SSD.
- **Everything needed is already prepared and committed** (branch `worktree-onsite-ingestion`):
  - Vehicle list: `scripts/seed_vehicles.json` (15 generations).
  - Driver: `scripts/ingest_seed_vehicles.py` — resumable, continues past per-vehicle failures, auto-retries NHTSA naming mismatches, emits paste-ready `ingestion_status.md` rows.
  - **Authoritative runbook: `docs/onsite_ingestion_runbook.md`** — prerequisites, hard rules, Path A (SSD-direct, recommended) vs Path B (LFS round-trip), verification, and a copy-paste AI task block for the second laptop.
- **Critical invariant**: keep `USE_GEMINI_EMBEDDINGS` unset/false on **every** machine. The store is 384-dim local MiniLM; mixing in 768-dim Gemini embeddings corrupts retrieval.
- **Python runtime note (deferred, optional)**: the earlier plan to bump `requires-python` to `>=3.12` for a runtime speedup is **not** a prerequisite for this batch — the pipeline runs fine on the locked 3.11 toolchain (`uv.lock`), and changing it mid-scale-up would only add risk. Treat it as a separate, later optimization.
...
| 2026-07-15 | Antigravity (Claude Sonnet 4.6 Thinking) | **Block 5: Mid-Repair Photo Checkpoint Pipeline (Stage 2.4) + "Check My ChatGPT Answer" Funnel (Stage 2.6)** | `backend/schemas.py`, `backend/services/gemini.py`, `backend/routers/repair.py`, `backend/routers/diagnose.py`, `frontend/src/lib/api.ts`, `frontend/src/app/check-ai/page.tsx` (NEW), `frontend/src/app/repair/page.tsx` (partial), `tests/unit/test_checkpoint.py` (NEW), `tests/unit/test_verify_external.py` (NEW) | **Backend fully complete and verified — 174 unit tests pass (163 pre-existing + 11 new), `ruff check` clean, `black --check` clean, `mypy` clean (34 source files, no issues).** Frontend partially complete: `/check-ai` page (Stage 2.6) written and type-safe; `api.postForm()` helper added to `api.ts`; `CameraIcon` imported + `checkpointStates` state + `verifyCheckpoint()` handler added to `repair/page.tsx` — but the actual **📸 Verify My Work button UI** (inside `renderStep`) was **NOT yet added** to the repair page before the session was interrupted. Frontend build NOT run yet (verify after adding the UI block). | **Block 5 backend complete. Frontend 80% complete — one remaining task.** Next agent must: **(1)** Add the `📸 Verify My Work` inline button + result panel inside the `renderStep` closure in `frontend/src/app/repair/page.tsx` (around line 640, just after the quality-checkpoint hint `div` that ends at ~L645). The button should: show `checkpointStates[i]?.status`, toggle open a hidden `<input type="file" accept="image/*" capture="environment">` ref per step, call `verifyCheckpoint(i, stepText, file)` on `onChange`, display a shimmer while verifying, then render a green ✅ or amber ⚠️ pill with `result.explanation`. **(2)** Run `cd frontend && pnpm run build` and confirm zero TypeScript errors. **(3)** Commit everything to `main` with message `feat: Block 5 — Mid-Repair Photo Checkpoint + Check-AI Funnel (Stages 2.4 & 2.6)`. **(4)** Update this `imp.md` log row's status. **(5)** Begin Block 6 per `docs/MODEL_ASSIGNMENT_GUIDE.md` if time permits. |
| 2026-07-15 | Claude Code (Sonnet 5) | **Block 5 completion: added the missing 📸 Verify My Work checkpoint UI (Stage 2.4) inside `renderStep` in `frontend/src/app/repair/page.tsx`.** The worktree for this task initially branched from a stale `origin/main` (several commits behind local `main`, missing the prior session's Block 5 backend work and `CameraIcon` import) — caught this via the resulting `'CameraIcon' is not defined'` build error, stashed the new UI edit, rebased the worktree branch onto local `main`, and reapplied the stash cleanly before rebuilding. | `frontend/src/app/repair/page.tsx` (added hidden per-step `<input type="file" accept="image/*" capture="environment">` wired to `checkpointFileRefs.current[i]`, a "📸 Verify My Work" button that triggers it and calls `verifyCheckpoint(i, stepText, file)` on change, a `.loading-spinner` shown while `checkpointStates[i]?.status === 'verifying'`, and a green/amber result pill rendering `result.explanation` per the 7-Pillars readiness color coding — plus a red error pill with a "Try again" reset), `docs/implementation/imp.md` (checked off Stage 2.4 and 2.6, updated progress table to 11/15). | `cd frontend && ./node_modules/.bin/next build` — compiles clean, zero TypeScript/ESLint errors (two pre-existing `react-hooks/exhaustive-deps` warnings in `repair/page.tsx`/`results/page.tsx` are unrelated to this change and were not introduced by it). Did not re-run backend `pytest`/`ruff`/`mypy` since no backend files were touched in this session. | **Block 5 (Stages 2.4 & 2.6) is now fully complete end-to-end.** Work committed on git branch `worktree-block5-checkpoint-ui`, not yet pushed/PR'd. Next agent should push and open a draft PR, then begin **Stage 3 (Maintenance Content / Knowledge Hub / Referral Program)** per `docs/MODEL_ASSIGNMENT_GUIDE.md`. |
| 2026-07-15 | Antigravity (Gemini 3.5 Flash) | **Block 7: High-Velocity Content & SEO Hub (Stages 3.1 & 3.2)** | `backend/repair_templates.py`, `backend/pricing.py`, `tests/unit/test_templates.py`, `frontend/src/app/hub/articles.ts`, `frontend/src/app/hub/page.tsx`, `frontend/src/app/hub/[slug]/page.tsx`, `frontend/src/app/globals.css` | All backend tests passed (181 passed). Static analysis (`ruff`, `black`, `mypy`) clean. Next.js production build compiles successfully with zero errors. | **Block 7 completed and verified.** Next agent begins **Block 8: Referral Program & Recall/TSB Watch** per `docs/MODEL_ASSIGNMENT_GUIDE.md`. |
| 2026-07-15 | Claude Code (Sonnet 5) | **Block 8: Growth & Proactive Retention Automation (Stage 3.3 -- Referral Program & Recall/TSB Watch Alerts).** Referral tracking: `DbUser` gains `referral_code` (unique, 8-char, auto-generated at signup via a DB-uniqueness-checked loop in `backend/routers/auth.py`; a Python-side random default on the column itself covers any direct `DbUser(...)` construction elsewhere, e.g. test fixtures, that doesn't pass one explicitly), `referred_by_code`, and `referral_credits` (int ledger). `POST /api/auth/request-link` now accepts an optional `referral_code`; on first-time account creation only, a valid code awards 1 credit to both the referrer and the new signup (an invalid/typo'd code is silently ignored rather than blocking signup). New `POST /api/auth/redeem-referral-credit` spends 1 credit to write a `DbGuideUnlock` row for a caller-chosen VIN -- reusing the exact unlock-verification path `POST /api/repair`/`/api/repair/chat` already trust from real Polar checkouts (`backend/routers/repair.py::_session_unlocks_vin`), so a credit-redeemed guide is indistinguishable from a paid one to the rest of the app. Recall/TSB watch: new `backend/scripts/recall_watch_cron.py`, a standalone async script (`uv run python -m backend.scripts.recall_watch_cron`) that groups every `DbSavedRepair` row by (make, model, year), queries `backend/services/nhtsa_safety.py::get_open_recalls` once per distinct vehicle (not per VIN, since NHTSA's `recallsByVehicle` only takes those three fields), and emails each affected owner via a new `send_recall_alert_email()` in `backend/services/email.py`. New `DbRecallAlertSent` table (composite PK `vin`+`campaign_number`) deduplicates so the same open recall -- which can stay open for years -- isn't re-emailed every single daily run; a VIN with no attached user still gets its recalls marked sent so a future owner isn't silently skipped and the run doesn't retry forever. Both email sends and the cron script never raise on failure (same fail-open contract as the rest of the NHTSA integration), just log and move to the next vehicle. | `backend/core/models.py` (`DbUser.referral_code`/`referred_by_code`/`referral_credits`, new `DbRecallAlertSent`), `backend/schemas.py` (`RequestLinkRequest.referral_code`, `UserResponse.referral_code`/`referral_credits`, `RedeemReferralCreditRequest`/`Response`), `backend/routers/auth.py` (referral-code generation/reward logic in `request_link`, new `redeem_referral_credit` endpoint), `backend/services/email.py` (`send_recall_alert_email`), `backend/scripts/__init__.py` (new), `backend/scripts/recall_watch_cron.py` (new), `tests/unit/test_auth.py` (6 new tests: unique code generation, valid-code dual-party reward, invalid-code no-op, credit redemption + double-redemption 402, auth-required 403), `tests/unit/test_recall_watch_cron.py` (new: 3 tests -- new-recall send, dedup-on-rerun, no-saved-vehicles no-op), `docs/implementation/imp.md`. | `uv run pytest tests/unit/` -- **189 passed** (181 pre-existing + 8 new). `uv run ruff check backend/`, `uv run black --check backend/`, `uv run mypy backend/` -- all clean. `./tests/verify_tests.sh` -- 5/5 (healthy pass + all 4 fault-injections correctly caught). Did not run `next build` -- zero frontend files were touched in this block (Claude/Gemini split: this was backend-only referral/cron work, no presentational surface to change), and the worktree has no `frontend/node_modules` installed. | **Block 8 (Stage 3.3) completed and verified -- Gap Analysis table now 15/15 (100%) except the on-site ingestion batch (Stage 4.1, see next entry).** No frontend UI was built for redeeming referral credits or surfacing `referral_code`/`referral_credits` on `/settings` -- that's presentational work for Gemini per the pinned Claude/Gemini split (`CLAUDE.md`), and the backend contract (`UserResponse.referral_code`/`referral_credits`, `POST /api/auth/redeem-referral-credit`) is now stable for it to build against. The cron script is not yet wired into an actual scheduler (cron/launchd) on any host -- that's an ops/deployment step, documented as a runnable module in the script's own docstring. |
| 2026-07-15 | Claude Code (Opus 4.8) | **Stage 4.1 prep pivot: Jules → on-site laptop ingestion.** No ingestion run on this 8GB Mac (the machine being offloaded). Instead prepared everything a more capable laptop needs to run the ~15-generation batch straight into the SSD-hosted store. Verified the ETL layer is fully portable (no hardcoded paths, `uv.lock` + `etl` group committed) and confirmed the critical cross-machine invariant: the live store uses ChromaDB's local **all-MiniLM-L6-v2 (384-dim)** embeddings (`USE_GEMINI_EMBEDDINGS=false`), which every machine must keep consistent or retrieval breaks. Live store is only ~123 MB; the 4.4 GB is the gitignored PDF workspace. Selected 15 vehicle generations aligned to the fraud-shield business model (highest US parc + highest-repair-cost / known-issue vehicles: full-size trucks, the top compact/midsize SUVs, Nissan-CVT and Hyundai/Kia-Theta-II known-issue nameplates), representative mid-generation years (2013–2017) for mature TSB volume, deduped against the 7 already-ingested. | `scripts/seed_vehicles.json` (replaced 8 baseline entries with the 15 new generations), `scripts/ingest_seed_vehicles.py` (hardened: argparse `--seed-file`, NHTSA naming auto-retry, DONE/SKIPPED/FAILED classification, paste-ready `ingestion_status.md` row output), `docs/onsite_ingestion_runbook.md` (NEW — authoritative runbook: Path A SSD-direct / Path B LFS round-trip, hard rules, verification, copy-paste AI task block), `docs/jules_ingestion_runbook.md` (deprecation banner), `docs/ingestion_status.md` (baseline vs 15-vehicle queue tables, fixed stale worktree path), `docs/implementation/imp.md` (Stage 4.1 rewrite + gap row), `docs/MODEL_ASSIGNMENT_GUIDE.md` (Block 9 → on-site), `docs/UPDATED_PRODUCT_NORTH_STAR.md` (§7/§11 wording + §12 decision-log entry). | `uv run ruff check` / `black --check` / `mypy` on `scripts/ingest_seed_vehicles.py` — all clean. `python scripts/ingest_seed_vehicles.py --help` and `python -m etl --help` run. `seed_vehicles.json` parses to 15 vehicles. No pytest impact (no `backend/`/`tests/` changes). Did NOT run a real ingestion (offloaded to the capable laptop by design). | **Stage 4.1 is prepped and ready to execute on the second laptop.** Committed on branch `worktree-onsite-ingestion` (off `main`, includes Block 8), pushed with a draft PR. Next action is the human's: pull this branch on the capable laptop and follow `docs/onsite_ingestion_runbook.md` (or hand its §7 task block to an AI there). |
