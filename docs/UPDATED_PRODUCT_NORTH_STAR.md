...
- Finish the committed ~15-generation on-site ingestion batch before any GTM push (was a Jules cloud pilot; now runs on a local laptop — see `docs/onsite_ingestion_runbook.md`)
...
- **On-site laptop ingestion (replaces Jules for the KB scale-up)** — the ~15-generation vector-DB batch runs on a more capable laptop you control, plugged into the SSD, driven by a local AI agent following `docs/onsite_ingestion_runbook.md`. Google Jules was dropped here: it proved unreliable and could only hand work back through a Git-LFS round-trip (it could never touch the SSD directly). A local run is simpler, faster, and fully reproducible with zero API keys (embeddings are local MiniLM).
...
- ~15-generation on-site ingestion batch must complete before any GTM push — this is the actual "done" criterion for Phase 1, not the feature list alone. Runs on a more capable laptop (not Jules); everything is prepped in `scripts/seed_vehicles.json` + `docs/onsite_ingestion_runbook.md`.
...
| **2026-07-15** | §7/§10/§11 Stage 4 (KB scale-up) | **Vector-DB scale-up ingestion moves from Google Jules (cloud) to an on-site run on a more capable laptop; scoped to a curated 15 vehicle generations.** | **Why**: Jules proved unreliable in practice, and its architecture forced a Git-LFS round-trip because a cloud VM can never touch the SSD that holds the live knowledge base. A laptop the owner controls can plug the SSD in and ingest **straight into the live store** — simpler, faster, no LFS size ceiling, and fully reproducible with **zero API keys** since embeddings are ChromaDB's local `all-MiniLM-L6-v2` (384-dim). **Load-bearing invariant carried into the runbook**: every machine (ingesting *and* serving) must keep `USE_GEMINI_EMBEDDINGS` unset/false — mixing 768-dim Gemini vectors into the 384-dim MiniLM store corrupts retrieval. The 15 generations were chosen for the fraud-shield business model (§1/§3): highest US on-road population + highest repair-cost / notorious-known-issue vehicles (full-size trucks, top compact/midsize SUVs, Nissan-CVT and Hyundai/Kia-Theta-II nameplates), at mid-generation years (2013–2017) for mature TSB volume. **Tradeoff**: the SSD must physically travel to the ingesting laptop for the recommended direct path (a Git-LFS fallback path is documented for when it can't). Prepared artifacts: `scripts/seed_vehicles.json`, `scripts/ingest_seed_vehicles.py`, `docs/onsite_ingestion_runbook.md` (`docs/jules_ingestion_runbook.md` kept but deprecation-bannered). |
