...
├── data/                          # GITIGNORED. As of this session, a symlink to SSD-hosted storage on both
│   │                              #   this worktree and the user's main checkout (see §6) -- chroma_db (vector
│   │                              #   store), etl_workspace (raw downloaded TSB PDFs), rapp.db (SQLite app DB)
...
- **`data/` as an external-drive symlink** (added this session): the ingested knowledge base outgrew comfortable local SSD/internal-disk headroom on the development machine; the fix keeps the git-tracked project on fast internal storage while only the (much larger) data directory lives on an external SSD, via a plain symlink — zero code changes required since all data access already went through relative `./data/...` paths or the `--workspace`/`CHROMA_DB_PATH` overrides.
...
- **Google Jules ingestion hand-off** (this session's primary in-flight initiative): the plan is to have Jules (Google's async cloud coding agent) run the heavy NHTSA-TSB ingestion workload in its own cloud VM — offloading CPU/RAM load from the local development machine — then hand the resulting vector-store data back via a Git LFS commit (`kb_export/chroma_db`, `.gitattributes`-tracked), which the local machine merges into its live SSD-hosted store via `etl/import_kb.py` (upsert-based merge, not a raw file overwrite, since ChromaDB's metadata file is shared across all collections). **Both the export and import scripts were built and integrity-verified this session**; a full precise Jules task runbook was the next planned deliverable as of this document's writing (not yet completed — see the session log below for exact status).
...
- **Ingestion queue as of this document**: Corolla, Highlander, Civic, Accord, Camry, F-150, and Lexus ES (300h) are ingested (~9,600 vector-store chunks total). This was a deliberately curated pilot set, explicitly stopped short of the full national-scope plan above pending the Jules/cloud-storage transition.
...
## 7. Session Log — Steps Taken This Session

This section is a factual record of what was done in the working session that produced the changes described above, for full traceability.
